...
当前原型已具备从照片上传、后台分析到人工复核和打印报告的完整主链路：系统依次执行视觉识别、规章检索、内部风险分级和整改建议，保存法规引用快照，并允许确认、修正、标记误报或人工补录。AI 原始结论与每次人工操作分开留档。项目提供单进程 Docker 部署、访问保护、写请求限流、数据备份恢复、可选内部 AI 复核、可重跑离线评测和明确标识的历史回放。宿舍知识库有 25 条可追溯法规记录。真实视觉效果仍待团队提供凭据或 YOLO 权重及现场样图验收。
...
测试使用临时数据库和临时上传目录，不读取真实模型凭据、不调用付费 API，也不会写入仓库的 `data/`。当前测试覆盖图片校验、视觉结构、知识导入/检索、后台步骤、规则分级、引用快照、失败重试、重启恢复、人工复核审计、评测指标、离线回放幂等性和安全报告渲染。真实模型服务需由团队配置凭据后另做手工验收。

## 评测与离线回放

真实评测集放在 Git 忽略的 `evaluation/dataset.json` 和 `evaluation/images/` 中。数据格式、来源权限、冒烟/验证集划分和命令见 [离线评测数据约定](evaluation/README.md)。`scripts/evaluate.py` 可以读取已有预测，也可以向已启动的真实服务逐张上传并等待终态，输出误报、漏报、精确率、召回率、失败率和端到端 P50/P95。当前 [评测记录](docs/evaluation.md) 使用 4 条无图片格式示例，只证明工具可运行，不代表模型效果。

断网演示前可向当前配置的数据库写入 4 份完整历史回放：

```powershell
.\.venv\Scripts\python.exe scripts\seed_demo.py
```

命令可重复执行，不会重复添加记录。回放使用程序生成的合成示意图和真实法规快照；历史页、报告页和图片上都明确标识“离线回放”，不会调用方舟、YOLO 或文本模型。回放为只读记录，并从业务仪表盘统计中排除。演示顺序与故障分支核对见 [演示验收清单](docs/demo-acceptance.md)。
...
- `backend/app/evaluation.py`：标注/预测校验、指标计算、真实服务采集和可追溯报告生成。
- `backend/app/demo_seed.py`：幂等生成 4 份离线历史回放及合成示意图。
...
- `scripts/`：生产启动、知识导入、数据备份、确认式恢复、评测和离线回放命令。
...
实施方案第 1–7 步的仓库工程已完成，下一步进入真实模型/现场样本评测、第 8 步材料打包与校赛演练。YOLO 运行时、权重和类别映射仍通过既有适配器边界接入，不影响复核、评测和报告数据结构。
